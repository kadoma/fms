...
static int
fmd_adm_get_caselist(fmd_adm_t *ap)
{
	mqd_t mqd = ap->adm_mqd;
	struct mq_attr attr;
	void *recvbuf = NULL;
...
int ret = 0;
//printf("fmadm faulty: before mq_send()\n");
	ret = mq_send(mqd, (void *)cmd, 13 * sizeof (char), 0);
//	sleep(1);
	if (ret < 0) {
...
//printf("fmadm faulty: after mq_send() ret=%d\n", ret);
...
//	fafc = (void *)(fafh + fafh->fafh_hdrsize);
...
//		fafc = (void *)(fafh + fafh->fafh_hdrsize + i * sizeof (faf_case_t));
