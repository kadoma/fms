...
fmd_list_mod(void)
{
	...
	struct dirent *dp;
	const char *p;
	char path[] = "/usr/lib/fm/fmd/plugins/";
	...
	while ((dp = readdir(dirp)) != NULL) {
		...
		if ((strncmp(dp->d_name, "agent.", 6) == 0)
		 && (strstr(dp->d_name, ".conf") != NULL))
			fmd_modname_print(dp->d_name);

		if ((strncmp(dp->d_name, "evtsrc.", 7) == 0)
		 && (strstr(dp->d_name, ".conf") != NULL))
			fmd_modname_print(dp->d_name);
	}
...
fmd_add_conf(char *filename, char *mod_name)
{
	...
	if (evtsrc) {
		...
		while (fgets(buf, sizeof(buf), fp1)) {
			if (ready && (strncmp(buf, "#version ", 9) == 0)) {
				sprintf(buffer, "%s%s", buffer, buf);
				continue;
			}
			if (strncmp(buf, "subscribe ", 10) == 0)
				...
				printf("Please input:\t"
				       "for example: fault.cpu.intel.cache\n"
				       "\t(press 'quit' to quit)\n\n");
				while (fgets(buff, sizeof(buff), stdin)) {
					if (strstr(buff, "quit") != NULL)
						break;
					else
						sprintf(buffer, "%ssubscribe %s", buffer, buff);
				}
				ready = 1;
				continue;
			}
...
{
	FILE *fp = NULL;
	char dir[] = "/usr/lib/fm/fmd/plugins/";
	char filename[50];
	char buf[256];
	char buffer[PATH_MAX];
	...
	if (strstr(mod_name, "evtsrc.") != NULL)
		evtsrc = 1;
