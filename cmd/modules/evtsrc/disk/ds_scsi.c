 ...
 */
static void
scsi_translate_error(void *p, uint_t *skeyp, uint_t *ascp,
		uint_t *ascqp)
{
	struct scsi_cmnd_io *iop = p;
	uint8_t *sense = iop->scsi_rqbuf;
	int resp_code;

	if (SCSI_STATUS_CHECK_CONDITION == iop->scsi_status) {
        	resp_code = sense[0] & 0x7f;
        	if (resp_code >= 0x72) {
            		*skeyp = (sense[1] & 0xf);
            		*ascp = sense[2];
            		*ascqp = sense[3];
...
//	int senselen = sizeof (struct scsi_extended_sense);
	...
	if (sip->si_cdblen == MODE_CMD_LEN_6) {
		...
		result = do_scsi_mode_select(sip->si_dsp->ds_fd,
			page_code, options, buf, buflen,
			&headers->ms_hdr.g0, &sense, senselen, p);
	} else {
		/* The following fields are reserved during mode select: */
		headers->ms_hdr.g1.ms_header.length = 0;
		headers->ms_hdr.g1.ms_header.device_specific = 0;

		result = do_scsi_mode_select_10(sip->si_dsp->ds_fd,
			page_code, options, buf, buflen,
			&headers->ms_hdr.g1, &sense, senselen, p);
	}

	if (result != 0)
		scsi_translate_error(p, skp, ascp, ascqp);
//		printf("scsi_translate_error()\n");
...
//	int senselen = sizeof (struct scsi_extended_sense);
	...
	if (sip->si_cdblen == MODE_CMD_LEN_6) {
		result = do_scsi_mode_sense(sip->si_dsp->ds_fd, page_code,
			pc, buf, buflen, &headers->ms_hdr.g0, &sense,
			senselen, p);
	} else {
		result = do_scsi_mode_sense_10(sip->si_dsp->ds_fd,
...
//	int sensebuflen = sizeof (struct scsi_extended_sense);
	uint8_t sense[32];
    	uint8_t sensebuf[18];
	int senselen = sizeof(sense);
	int sensebuflen = sizeof(sensebuf);
	int result;
	void *p;
...
//	bzero(&sensebuf, sizeof (struct scsi_extended_sense));
	...
	result = do_scsi_request_sense(sip->si_dsp->ds_fd,
		(caddr_t)&sensebuf, sensebuflen, &sense, senselen, p);
...
//	int senselen = sizeof(struct scsi_extended_sense);
	...
	result = do_scsi_log_sense(sip->si_dsp->ds_fd, page_code, page_control,
		page_data, page_size, &sense, senselen, p);
...
load_logpages(ds_scsi_info_t *sip)
{
	...
	if (result == 0) {
		...
		printf("failed to get log pages "
...
static void *
ds_scsi_open(disk_status_t *dsp)
{
	ds_scsi_info_t *sip;
...
ds_transport_t ds_scsi_transport = {
	ds_scsi_open,
	ds_scsi_close,
	ds_scsi_scan
};
